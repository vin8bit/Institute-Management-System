...
public class UpdateStaff extends JFrame implements ActionListener {
	...
	java.time.format.DateTimeFormatter format = java.time.format.DateTimeFormatter.ofPattern("dd-MM-yyyy");  
		...
		public UpdateStaff(){
			...
			l17 = new JLabel("Designation:");
			l17.setBounds(430,240,100,20);
	...
	{	if(e.getItemSelectable()==box2){ ltf6.setText("Female"); }
		...
		choice4.addItemListener(new ItemListener()
				{
					public void itemStateChanged(ItemEvent v)
					{
						ltf17.setText((String)choice4.getSelectedItem());
					}
				});
		...
		panel2.add(choice4);			
			...
			JLabel limg = new JLabel(new ImageIcon("images/people.png"));
			limg.setBounds(460,260,280,160);
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==submit){
			...
			if(s1.isEmpty()||s2.isEmpty()||s3.isEmpty()||s4.isEmpty()||s5.isEmpty()||s6.isEmpty()||s7.isEmpty()||s8.isEmpty()||s9.isEmpty()||s10.isEmpty()||s11.isEmpty()||s12.isEmpty()||s13.isEmpty()||s14.isEmpty()||s15.isEmpty()||s16.isEmpty()||s17.isEmpty() ){
				...
				try        { 		Class.forName("oracle.jdbc.driver.OracleDriver");
							...
							stm =  con.prepareStatement("insert into vstaff values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			                		stm.setString(1,s1);
							stm.setString(2,s2);
							stm.setString(3,s3);	
							stm.setString(4,s4);
					...
					box1.setSelected(true);
							...
							con.close();
								...
								if(ph3==1){JOptionPane.showMessageDialog(this,"Staff ID only number accepted","Error",JOptionPane.ERROR_MESSAGE);}
								} 						
							}
							else{ if(ch1==1){JOptionPane.showMessageDialog(this,"Staff ID already added","Error",JOptionPane.ERROR_MESSAGE);}}					
						...
						if(!dl.isEmpty()){
							...
							if(de==1){ 
								...
								try{	
									...
									String sk = ltf6.getText();
									String sk2 = ltf9.getText();
									String sk3 = ltf12.getText();
									String sk4 = ltf17.getText();
									if(sk.equalsIgnoreCase("Male")){ box1.setSelected(true);}
									if(sk.equalsIgnoreCase("Female")){ box2.setSelected(true);}
									choice5.select(sk2);
									choice3.select(sk3);
									choice4.select(sk4);
									con.close();
